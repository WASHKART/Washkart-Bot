...
// of WhatsApp's transient media storage and 24h-window failure modes.
async function uploadPhotoToStorage(buffer, orderId, kind) {
  const path = `${orderId}/${kind}-${Date.now()}.jpg`;
  await axios.post(`${SUPABASE_URL}/storage/v1/object/order-photos/${path}`, buffer, {
...
async function appendOrderPhoto(orderId, url, kind, caption) {
  try {
    ...
    photos.push({ url, kind, caption: caption || "", at: new Date().toISOString() });
    await dbUpdate("bookings", `order_id=eq.${orderId}`, { photos });
  } catch (e) { console.error("[appendOrderPhoto] failed:", e.message); }
}
...
app.post("/pickups/:orderId/delivery-photo", async (req, res) => {
  ...
  try {
    ...
    uploadPhotoToStorage(buffer, orderId, mode).then(url =>
      appendOrderPhoto(orderId, url, mode, mode === "left_outside" ? "Left outside proof" : "UPI payment proof")
    ...
    // Wrapped separately — if the customer's 24h WhatsApp window is closed, that failure
    // must NOT prevent the owner's copy below from being attempted. Owner's copy is the
    // business's own record and matters more than the customer nicety.
    if (b.phone) {
      try {
        await sendImageToCustomer(b.phone, mediaId, custCaption, numId);
      } catch (custErr) {
        console.error(`[delivery-photo] Customer send failed for ${orderId} (likely 24h window closed):`, custErr?.response?.data || custErr.message);
      }
    }
...
app.post("/pickups/:orderId/report-issue", async (req, res) => {
  ...
  try {
    ...
    if (photoBase64) {
      ...
      uploadPhotoToStorage(buffer, orderId, "issue").then(url =>
        appendOrderPhoto(orderId, url, "issue", label)
      ).catch(e => console.error("[storage] issue photo upload failed:", e.message));
    } else {
...
app.post("/pickups/:orderId/taken-by-customer", async (req, res) => {
  ...
  try {
    ...
    if (photoBase64) {
      ...
      uploadPhotoToStorage(buffer, orderId, "taken").then(url =>
        appendOrderPhoto(orderId, url, "taken", paidMethod ? `Taken by customer — paid ${paidMethod}` : "Taken by customer")
      ).catch(e => console.error("[storage] taken-by-customer photo upload failed:", e.message));
    }
